...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    // 宣告 Shared Memory
    __shared__ unsigned char smem[TILE_H + 2 * RADIUS][TILE_W + 2 * RADIUS][3];
    ...
    // Shared Memory 
    int sm_w = blockDim.x + 2 * RADIUS;
    int sm_h = blockDim.y + 2 * RADIUS;
    int sm_size = sm_w * sm_h;

    for (int i = tid; i < sm_size; i += blockSize) {
        int sm_y = i / sm_w;
        int sm_x = i % sm_w;

        int global_x = blockIdx.x * blockDim.x + sm_x - RADIUS;
        int global_y = blockIdx.y * blockDim.y + sm_y - RADIUS;

        if (global_x >= 0 && global_x < width && global_y >= 0 && global_y < height) {
            int idx = (global_y * width + global_x) * channels;
            smem[sm_y][sm_x][0] = s[idx + 0]; // B
            smem[sm_y][sm_x][1] = s[idx + 1]; // G
            smem[sm_y][sm_x][2] = s[idx + 2]; // R
        } else {
            smem[sm_y][sm_x][0] = 0;
            smem[sm_y][sm_x][1] = 0;
            smem[sm_y][sm_x][2] = 0;
    ...
    for (int i = 0; i < Z; ++i) {
        for (int v = -yBound; v <= yBound; ++v) {
            for (int u = -xBound; u <= xBound; ++u) {
                unsigned char R = smem[threadIdx.y + v + RADIUS][threadIdx.x + u + RADIUS][2];
                unsigned char G = smem[threadIdx.y + v + RADIUS][threadIdx.x + u + RADIUS][1];
                unsigned char B = smem[threadIdx.y + v + RADIUS][threadIdx.x + u + RADIUS][0];
...
int main(int argc, char **argv) {
    ...
    dim3 blocks((width + TILE_W - 1) / TILE_W, (height + TILE_H - 1) / TILE_H);

    // launch cuda kernel
    sobel<<<blocks, threads>>>(dsrc, ddst, height, width, channels);
