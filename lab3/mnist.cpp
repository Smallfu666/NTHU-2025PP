...
{
    #pragma acc parallel loop present(A[0:n*k], B[0:k*m], C[0:m], D[0:n*m]) gang vector_length(256)
    for (int i = 0; i < n; i++) {
...
void Sigmoid(float *A, int n, int m) {
    #pragma acc parallel loop present(A[0:n*m]) gang vector_length(256)
    for (int i = 0; i < n; i++) {
        #pragma acc loop vector
        for (int j = 0; j < m; j++) {
            // Sigmoid(x) = 1/(1+exp(x))
...
void Argmax(float *A, int *D, int n, int m) {
    #pragma acc parallel loop present(A[0:n*m], D[0:n]) gang vector_length(256)
    for (int i = 0; i < n; i++) {
        int max_idx = 0;
        float mx = A[i * m];
