...
int main(int argc, char** argv) {
    ...
    size_t img_size = height * width * channels * sizeof(unsigned char);
    CUDA_CHECK(cudaMalloc(&device_s, img_size));
    CUDA_CHECK(cudaMalloc(&device_t, img_size));

    /* Hint 2 */
    // cudaMemcpy(...) copy source image to device (filter matrix if necessary)
    CUDA_CHECK(cudaMemcpy(device_s, host_s, img_size, cudaMemcpyHostToDevice));
    /* Hint 3 */
    ...
    CUDA_CHECK(cudaPeekAtLastError());     // 檢查啟動是否成功
    CUDA_CHECK(cudaDeviceSynchronize()); // 等待 Kernel 執行完畢並檢查執行時錯誤
    /* Hint 4 */
    // cudaMemcpy(...) copy result image to host
    CUDA_CHECK(cudaMemcpy(host_t, device_t, img_size, cudaMemcpyDeviceToHost));
